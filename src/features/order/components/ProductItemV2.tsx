...
export default function ProductItemV2({ item, setProductId }: { item: ProductDetailType, setProductId: Dispatch<SetStateAction<string>> }) {
  const { listProductChose, handleRemoveProduct } = useContextValid(OrderContext)
  return (
   <TouchableOpacity delayLongPress={300} onPress={() => setProductId(item.id)}>
